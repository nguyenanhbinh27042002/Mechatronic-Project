...
class DisplayDurian :
    ...
    def readImage(self,img):
        # chuyen anh mau ban dau thanh mang
    ...
    # Loc nhieu va loai bo nhieu
    def removeNoise(self):
        #Tạo mảng có kích thước 3x3 với giá trị 1
        self.kernel = np.ones((3, 3), np.uint8)
        #loại bỏ nhiễu bên ngoài cần xác định(dùng Open), iterations(lặp đi lặp lại)
        opening = cv.morphologyEx(self.thresh, cv.MORPH_OPEN, self.kernel, iterations=2)
        self.curImg = opening

    def DisplayErrorSurfce(self):
        ...
        SurfaceImage = cv.cvtColor(self.Img, cv.COLOR_HSV2BGR)
        self.curImg = SurfaceImage