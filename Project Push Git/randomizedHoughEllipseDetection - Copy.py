...
# Test the class
image = np.random.rand(256, 256) * 255  # Generating a random image
mask = np.zeros_like(image, dtype=np.uint8)  # Creating a mask, here it's all zeros
mask[100:150, 100:150] = 255  # Defining a region of interest (ROI) in the mask
ellipse_finder = FindEllipseRHT(image, mask)
ellipse_finder.run(debug_mode=False, plot_mode=True)
